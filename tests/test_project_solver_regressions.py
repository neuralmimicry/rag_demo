...
def test_requirements_only_does_not_add_keyword_derived_accuracy_requirement(monkeypatch, tmp_path):
    project_root = tmp_path / "contract-project"
    project_root.mkdir()
    requirements_path = project_root / "requirements.md"
    requirements_path.write_text(
        "REQ-001: Validate subprocess output mismatches and report success.\n",
        encoding="utf-8",
    )
    output_path = project_root / "project_solution.json"
    ...
    report = json.loads(output_path.read_text(encoding="utf-8"))
    register = report["requirements_register"]["requirements"]
    assert [item["id"] for item in register if item.get("source") != ["global"]] == ["REQ-001"]
    assert not any("accuracy or prediction quality" in item["description"].lower() for item in register)
