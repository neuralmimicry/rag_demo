...
- `[x]` Rewired [refiner_web.py](/home/pbisaacs/Developer/neuralmimicry/rag_demo/refiner_web.py) to register the extracted handlers
- `[x]` Deleted the superseded inline assistant/RAG route bodies from [refiner_web.py](/home/pbisaacs/Developer/neuralmimicry/rag_demo/refiner_web.py)
...
- `[x]` Extended [refiner_central_store.py](/home/pbisaacs/Developer/neuralmimicry/rag_demo/refiner_central_store.py) to expose:
  - `assistant_conversations`
  - `assistant_episodes`
  - `assistant_traces`
  - `assistant_semantic_cache`
  - `rag_metadata`
- `[x]` Added Postgres-backed dual-write for assistant episodic memory in [refiner_web.py](/home/pbisaacs/Developer/neuralmimicry/rag_demo/refiner_web.py)
- `[x]` Added Postgres-backed fallback reads for assistant episodic memory in [refiner_web.py](/home/pbisaacs/Developer/neuralmimicry/rag_demo/refiner_web.py)
...
- [refiner_web.py](/home/pbisaacs/Developer/neuralmimicry/rag_demo/refiner_web.py)
...
- `[x]` Leave [refiner_web.py](/home/pbisaacs/Developer/neuralmimicry/rag_demo/refiner_web.py) with compatibility shims only
...
- `[x]` Move `_coerce_rag_sources` out of [refiner_web.py](/home/pbisaacs/Developer/neuralmimicry/rag_demo/refiner_web.py)
- `[x]` Move `_build_rag_documents` out of [refiner_web.py](/home/pbisaacs/Developer/neuralmimicry/rag_demo/refiner_web.py)
...
- `[ ]` Add atomic active-version pointer updates in Postgres
...
- `[ ]` Add explicit refusal policies for unsafe tool use
...
- `central_store/assistant.py` now provides `nm_assistant_semantic_cache` via `PostgresAssistantSemanticCacheStore`, exposed through [refiner_central_store.py](/home/pbisaacs/Developer/neuralmimicry/rag_demo/refiner_central_store.py).
...
- `[~]` Add reranking and coverage grading
- `[x]` Retry once with decomposed sub-queries before refusing
- `[ ]` Bind answer claims to exact source chunks and locators
...
- `assistant_pipeline/retrieval/coverage_grader.py` and `assistant_pipeline/retrieval/retrieval_planner.py` now exist and are wired through [assistant_pipeline/service.py](/home/pbisaacs/Developer/neuralmimicry/rag_demo/assistant_pipeline/service.py).
...
- The first self-correcting retrieval rollout is also conservative:
  ...
  - performs at most one retry/decomposition pass
...
- Reranking, richer decomposition, tighter publication/mirroring semantics, and citation claim binding remain follow-up work.
- Added focused regression coverage in:
...
- `[ ]` Add golden evaluation fixtures for query rewriting, citation coverage, and refusal correctness
- `[ ]` Add rollout checks covering both file-backed and Postgres-backed assistant memory paths
...
1. Add reranking and richer decomposition so the retry loop can do more than exact clause splitting.
2. Tighten the RAG publish path so compatibility mirroring and active-version publication are coordinated more explicitly across filesystem and Postgres.
3. Bind answer claims to exact chunks and locators for end-to-end citation auditing.
4. Extend the security envelope to cover unsafe tool-use refusal policies and rollout telemetry.
