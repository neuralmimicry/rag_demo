...
from refiner.document_schema import DocumentElement, coerce_document_elements, format_locator, format_source_citation
...
class RagStore:
    ...
    def mirror_index_artifact(self, owner: str, name: str, artifact_path: str) -> str:
        """Mirror an immutable version artefact into the legacy active path."""

        source_path = str(artifact_path or "").strip()
        if not source_path or not os.path.exists(source_path):
            return ""
        target_path = self._index_path(owner, name)
        self._copy_file_atomic(source_path, target_path)
        source_dense_path = dense_artifact_path_for_index(source_path)
        target_dense_path = dense_artifact_path_for_index(target_path)
        if source_dense_path and os.path.exists(source_dense_path):
            self._copy_file_atomic(source_dense_path, target_dense_path)
        elif target_dense_path and os.path.exists(target_dense_path):
            os.remove(target_dense_path)
        return target_path
