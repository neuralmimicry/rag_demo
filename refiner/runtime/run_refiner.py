...
import sys
import time
from refiner.logging_utils import setup_logging
...
def _run_jira_workflow() -> int:
    ...
    from refiner.main import main as jira_main
    jira_main()
    return 0
...
def _run_confluence_analysis(
    ...
    from refiner.credentials import get_credentials, get_llm_credentials
    from refiner.confluence_analysis import analyze_space_and_write_report
...
def _run_jira_analysis(
    ...
    from refiner.config_loader import load_config
    from refiner.credentials import get_credentials, get_llm_credentials
    from refiner.jira_analysis import analyze_jira_and_write_report
...
def _run_project_solver(
    ...
    from refiner.project_solver import run_project_solver
...
def _run_delivery_pipeline(
    ...
    if config_path and str(config_path).strip().lower() == "default":
        resolved_config = os.path.join(PROJECT_ROOT, "delivery_pipeline.json")
    else:
        resolved_config = config_path or os.path.join(project_root, "delivery_pipeline.json")
    if not os.path.exists(resolved_config):
        fallback = os.path.join(PROJECT_ROOT, "delivery_pipeline.json")
    ...
    try:
        if args.topic_research:
            if args.clear_research_cache:
                from refiner.topic_researcher import RESEARCH_CACHE_ROOT
                import shutil
