...
def _extract_workflows(root: str) -> List[Dict[str, Any]]:
    path = os.path.join(root, "refiner", "run_refiner.py")
                ...
                workflows.append(
                    {
                        ...
                        "source": "refiner/run_refiner.py:docstring",
                    }
                )
...
def _extract_agentic_phases(root: str) -> List[str]:
    path = os.path.join(root, "refiner", "agentic_workflow.py")
    if not os.path.exists(path):
...
def analyse_repo(root: str) -> Dict[str, Any]:
    ...
    routes = _extract_routes(os.path.join(root, "refiner", "refiner_web.py"))
    report = {
