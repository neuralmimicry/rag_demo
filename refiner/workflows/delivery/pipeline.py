...
from refiner.mcp_client import MCPClient, MCPServerConfig
...
from refiner.project_solver import _find_workspace_venv, run_project_solver
from refiner.language_detector import detect_languages
from refiner.repo_context import RepoIndex
...
def _pipeline_head_sha(project_root: str) -> Optional[str]:
    """Return the exact commit that delivery must prove in GitHub Actions."""
...
def _github_actions_report(
    ...
    *,
    ...
    # Query queued/in-progress runs too: delivery must wait for the workflow
    # generated by this push, rather than accidentally accepting an older run.
    query = urllib.parse.urlencode(
        {"branch": branch, "per_page": 50},
    ...
    # Without an exact commit there is nothing safe to wait for or approve.
    # Keep direct helper calls fail-closed and immediate; normal delivery calls
    # always obtain HEAD after the VCS actions above.
    wait_timeout = 0.0 if not expected_sha else max(
        0.0,
        float(cfg.get("wait_timeout_sec", cfg.get("timeout_sec", 900)) or 0),
    )
    poll_interval = max(0.1, float(cfg.get("poll_interval_sec", 10) or 10))
    ...
    while True:
        try:
            ...
            report["reason"] = (
                f"GitHub Actions workflow {workflow_file} is unavailable for {owner}/{repo}"
                if exc.code == 404
                else f"unable to verify GitHub Actions workflow {workflow_file}: {detail}"
            )
            return report
        except Exception as exc:
            report["checked"] = True
        ...
        if expected_sha:
            matching_runs = [
                ...
                if str(run.get("head_sha") or "").strip().lower() == expected_sha.lower()
            ]
            run = matching_runs[0] if matching_runs else None
        else:
            # Direct callers without a commit context retain the old behavior,
            # while the delivery pipeline always supplies expected_sha.
        ...
        if run is not None:
            ...
            if status == "completed":
                ...
                if report["succeeded"]:
                    report["reason"] = (
                        f"GitHub Actions workflow {workflow_file} succeeded for commit "
                        f"{expected_sha or run.get('head_sha')} on {owner}/{repo} branch {branch}"
                    )
                else:
                    report["reason"] = (
                        f"GitHub Actions workflow {workflow_file} concluded with "
                        f"{conclusion or 'unknown'} for commit {expected_sha or run.get('head_sha')}"
                    )
                return report
        ...
        if time.monotonic() >= deadline:
            ...
            if expected_sha:
                report["reason"] = (
                    f"timed out waiting for GitHub Actions workflow {workflow_file} for commit "
                    f"{expected_sha} on {owner}/{repo} branch {branch}"
                )
            else:
                report["reason"] = (
...
def run_delivery_pipeline(
    ...
    *,
    ...
    expected_sha = _pipeline_head_sha(project_root) if allow_run else None
    github_actions_report = _github_actions_report(
        project_root,
        config.github_actions or {},
        allow_run,
        expected_sha=expected_sha,
    )
