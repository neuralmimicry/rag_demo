...
from refiner.llm_providers import get_provider, LLMProvider, LLMResponse, LLMQuotaError, LLMError
from refiner.agentic_workflow import AgenticWorkflow, PhaseResult, ProgressTracker
from refiner.jira_analysis import fetch_issues as jira_fetch_issues, _jira_get, _extract_confluence_ids
from refiner.confluence_analysis import _conf_get, PageInfo
from refiner.file_converter import FileConverter
from refiner.web_research import (
    BraveSearchEngine,
...
from refiner.refiner_ai_orchestration import describe_provider, orchestrate_provider_candidates
...
class TopicResearcher:
    ...
    def __init__(
        ...
        orchestrated_candidates = [
            provider
            for provider in (self.llm, self.fallback_llm)
            if provider is not None and not isinstance(provider, _NullLLMProvider)
        ]
        if orchestrated_candidates and self.fallback_llm is None:
            # Topic research already has an explicit primary->fallback contract.
            # Keep that path deterministic when a dedicated fallback provider is
            # configured instead of layering concurrent orchestration on top.
            self.llm = (
    ...
    def _init_role_providers(self) -> None:
        for role, cfg in self.agentic_roles.items():
            ...
            orchestrated_candidates = [
                candidate
                for candidate in (provider, self.fallback_llm)
                if candidate is not None and not isinstance(candidate, _NullLLMProvider)
            ]
            if orchestrated_candidates:
                provider = (
                    orchestrate_provider_candidates(
                        orchestrated_candidates,
                        workflow="topic_research",
    ...
    def _predict_with_fallback(
        ...
        **kwargs,
        ...
        if getattr(primary, "refiner_orchestrated", False) is True:
            response = primary.predict(messages, system=system, **kwargs)
            self._record_orchestrated_quota_signal(response)
            return response
    ...
    def _record_quota_error(self, error: Exception) -> None:
        """Track quota state across direct and wrapped provider failure paths."""
        if isinstance(error, LLMQuotaError):
            self._quota_reached = True

    def _record_orchestrated_quota_signal(self, response: Optional[LLMResponse]) -> None:
        """Preserve quota circuit-breaker state when orchestration succeeds via fallback."""
        raw = response.raw if isinstance(getattr(response, "raw", None), dict) else {}
        refiner_ai = raw.get("refiner_ai") if isinstance(raw.get("refiner_ai"), dict) else {}
        for candidate in refiner_ai.get("candidates") or []:
            if not isinstance(candidate, dict):
                continue
            if str(candidate.get("error_class") or "").strip() == "LLMQuotaError":
                self._quota_reached = True
                return
            ...
            try:
                if is_jira_confluence:
                    ...
                    if "/browse/" in parsed.path:
                        ...
                        from refiner.jira_analysis import _map_issue
    ...
    def _evaluate_issue_richness(self, issue: Any) -> Dict[str, Any]:
        ...
        if updated:
            if isinstance(updated, str):
                try:
                    from refiner.atlassian_utils import parse_atlassian_datetime
