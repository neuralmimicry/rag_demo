...
JOB_ROOT = os.getenv("RAG_DEMO_JOB_DIR", os.path.join(BASE_DIR, "job_data"))
...
DEFAULT_FORK_ORG = os.getenv("RAG_DEMO_FORK_ORG", "neuralmimicry")
DEFAULT_WORKERS = int(os.getenv("RAG_DEMO_WORKERS", "2"))
DEFAULT_TAIL = int(os.getenv("RAG_DEMO_LOG_TAIL", "200"))
DEFAULT_REPO_PRIVATE = str(os.getenv("RAG_DEMO_REPO_PRIVATE", "1")).lower() in {"1", "true", "yes"}
REQUIREMENTS_MAX_BYTES = int(os.getenv("RAG_DEMO_REQUIREMENTS_SCAN_BYTES", "20000"))

app = Flask(__name__, static_folder="web/static", template_folder="web/templates")
app.secret_key = os.getenv("RAG_DEMO_SECRET_KEY") or os.urandom(32)
...
class UserStore:
    ...
    def ensure_admin_from_env(self) -> None:
        admin_user = (os.getenv("RAG_DEMO_ADMIN_USER") or "").strip()
        admin_pass = (os.getenv("RAG_DEMO_ADMIN_PASS") or "").strip()
...
class JobManager:
    ...
    def _build_command(self, job: Job) -> List[str]:
        ...
        command = [
            os.getenv("RAG_DEMO_PYTHON", sys.executable),
            os.path.join(BASE_DIR, "run_rag.py"),
            "--log-file",
            job.log_path,
            "--emit-events",
            "--events-file",
    ...
    def _prepare_repo(self, job: Job) -> None:
        ...
        branch_name = (payload.get("work_branch") or f"rag-demo/{job.job_id[:8]}").strip()
    ...
    def _finalize_repo(self, job: Job) -> None:
        ...
        if not self._git_has_changes(workspace, job):
            ...
            return
        author_name = job.payload.get("git_author_name") or "rag-demo-bot"
        author_email = job.payload.get("git_author_email") or "automation@neuralmimicry.ai"
        commit_message = job.payload.get("commit_message") or f"Rag Demo updates ({job.job_id[:8]})"
    ...
    def _ensure_repo_exists(
        ...
        body = {
            ...
            "auto_init": True,
            "description": payload.get("repo_description") or "Rag Demo starter repo",
        }
...
def _require_login() -> Optional[Response]:
    ...
    if path.startswith("/static/") or path.startswith("/favicon"):
        return None
    if path in {"/login", "/setup"} or path.startswith("/api/health"):
        return None
...
if __name__ == "__main__":
    host = os.getenv("RAG_DEMO_HOST", "127.0.0.1")
    port = int(os.getenv("RAG_DEMO_PORT", "5001"))
    debug = os.getenv("RAG_DEMO_DEBUG", "0") in {"1", "true", "yes"}
