...
- Multi-language support: language/build-system detection is exported as `PIPELINE_LANGUAGES` and `PIPELINE_BUILD_SYSTEMS` for stage scripts, covering Python, Node/JS/TS, Go, Rust, C/C++, Kotlin (Gradle/Maven), Fortran, Pascal, Bash, and PowerShell. The solver adds language-appropriate acceptance checks, including native C/C++ configure/build or compiler checks and Kotlin build/test or compiler checks.
- GitHub Actions delivery gate: when `github_actions.enabled` and `require_success` are true, a production rollout waits for a completed successful workflow run whose `head_sha` exactly matches the commit produced by the VCS actions. Configure `wait_timeout_sec` and `poll_interval_sec` to tune the wait; a missing or mismatched run fails closed.
